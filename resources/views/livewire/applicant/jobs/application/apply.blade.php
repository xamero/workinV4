<div>
    @if ($activeVacancy != null)
        ...
        <div class="card bg-white shadow-lg border-0">
            <div class="card-header bg-white shadow-lg border-bottom-0">
                    ...
                    <div class="col-md-12">
                        @if ($activeVacancy->company != null)
                            @if ($activeVacancy->company->logo != null)
                                <img src="{{ asset('storage/company/' . $activeVacancy->company->logo) }}"
                                    class="mb-3" alt="{{ $activeVacancy->company->name }}" style="max-width: 8rem">
                            @endif
                        @endif
                        <p class="fs-4 text-primary fw-bold">{{ $activeVacancy->title }}</p>
                    </div>
                </div>
            </div>
            <div class="card-body ">
                <div class="card bg-white border-0">
                    <div class="card-body ">
                            ...
                            <div class="alert alert-info">
                                @foreach ($isApplicationExists->applicantProfile as $applied)
                                    @if ($applied->status == 1)
                                        ...
                                        {{ $activeVacancy->company->name ?? '' }} has responded to your job application.
                                        Please check your
                                        registered email.
                                    ...
                                    @else
                                        ...
                                        {{ $activeVacancy->company->name ?? '' }} has yet to respond to your job
                                        application.
                                    @endif
                                @endforeach
                            </div>
                        ...
                        <p class="mb-0"><span class="fw-bold">Location:</span> {{ $activeVacancy->location }}</p>
                        @if ($activeVacancy->salary_from != null or $activeVacancy->salary_to != null)
                            <p class="mb-0"><span class="fw-bold">Salary Range:</span>
                                &#8369;{{ number_format($activeVacancy->salary_from, 2) }}
                                @if ($activeVacancy->salary_to != null)
                                    - &#8369;{{ number_format($activeVacancy->salary_to, 2) }}
                                @endif
                            </p>
                        @endif
                        <p class="mb-0"><span class="fw-bold">Type:</span> {{ $activeVacancy->job_type }}</p>
                        <p class="mb-0"><span class="fw-bold">Total Vacancy:</span> {{ $activeVacancy->total_vacancy ?? '1' }}</p>
                        <p class="fw-bold">Qualifications and other details:</p>
